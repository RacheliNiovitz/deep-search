...
```bash
cd backend && dotnet test     # 15 בדיקות
```
...
- **`MetricField`** קיים במודל לצורך הרחבה עתידית, אך כרגע האגרגציות מתבצעות על `MonthlyIncome`. במערכת אמיתית, נתונים כספיים היו נשמרים כ-`decimal`.
- **ניהול State ב-Angular**: האפליקציה אינה משתפת state מורכב בין דפים, לכן **signals** מקומיים מספיקים — אין צורך ב-NgRx/Store (היו over-engineering ל-PoC בהיקף הזה). בהרחבה עתידית ניתן להוסיף store מרכזי.
